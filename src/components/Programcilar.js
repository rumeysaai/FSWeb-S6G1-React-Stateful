...
export default function Programcilar() {
  ...
  const [oneCikanlar, setOneCikanlar] = useState(enIyilerListesi);
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: oneCikanId === null ? "royalblue": "gold", // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          oneCikanlar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => { setOneCikanId(dev.id) /* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
            </div>
          )
        }
      </div>
